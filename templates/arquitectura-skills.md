...
Reglas: el coordinador queda fino (decide qué se reparte y cómo se junta); el laburo pesado vive en los skills/subagentes que llama. Paralelo solo cuando los pedazos son de verdad independientes; si cada paso necesita el anterior, es secuencial. Se aprende igual que un skill: usás uno de ejemplo (el de PPT) para ver el patrón y después armás el tuyo. El multi-agente brilla cuando una decisión necesita varias miradas (ej. el `council` del catálogo: 5 lentes en paralelo + peer-review anónimo + síntesis).
